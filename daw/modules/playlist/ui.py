...
class DAW_MT_PLPlayback(Menu):
    ...
    def draw(self, context):
        ...
        row.operator("daw.pl_stop", text="Stop", icon='STOP')
        layout.separator()
        row = layout.row(align=True)
        row.operator("daw.pl_toggle_record", text="Record", icon='REC')
        row.prop(pb, "metronome_enabled", text="Metrônomo", icon='METRONOME')
        layout.separator()
...
class DAW_PT_Playlist(Panel):
    ...
    def draw(self, context):
        ...
        row.operator("daw.pl_stop", text="⏹", icon='STOP')
        row.operator("daw.pl_toggle_record", text="●", icon='REC')
        row.prop(pb, "metronome_enabled", text="", icon='METRONOME')
